...
export default function AdminDashboard({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen text-neutral-500"><Loader2 className="w-6 h-6 animate-spin mr-2" /> Ładowanie podglądu...</div>;
  ...
  return (
    <div className="min-h-screen bg-neutral-100 p-8 font-sans print:p-0 print:bg-white">
      <div className="max-w-[90rem] mx-auto space-y-6 print:space-y-4 print:max-w-none">
        
        {/* Print Only Intro */}
        <div className="hidden print:block mb-8">
            <h1 className="text-3xl font-bold text-neutral-900 mb-2">{template.title} - Raport Indywidualny</h1>
            <div className="text-sm text-neutral-500 mb-6 flex justify-between border-b pb-4">
               <div>
            ...
            <p className="text-justify text-neutral-700 leading-relaxed mb-6">
        ...
        <header className="flex items-center justify-between bg-white p-6 rounded-xl shadow-sm border border-neutral-200 print:hidden">
            <div className="flex items-center gap-4">
                ...
                <div className="hidden md:block">
                     <Image src="/favicon.svg" alt="Logo" width={40} height={40} className="h-10 w-auto" />
                </div>
                <div>
                    <h1 className="text-2xl font-bold text-neutral-900">Podgląd na żywo: {template.title}</h1>
                    <p className="text-neutral-500 font-mono text-xs mt-1">ID Sesji: {token}</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                ...
                <div className="text-right">
                    <div className="text-3xl font-bold text-neutral-900">{progress}%</div>
                    <div className="text-xs text-neutral-500 uppercase font-semibold">Ukończono</div>
                </div>
            </div>
        </header>

        {/* Demographics Config - Hide in Print if static data is shown above */}
        <Card className="border-0 shadow-sm ring-1 ring-neutral-200 print:hidden">
            <CardHeader className="pb-3 border-b border-neutral-50">
                <CardTitle className="text-sm font-medium text-neutral-900">Konfiguracja norm (Steny)</CardTitle>
            </CardHeader>
            <CardContent className="pt-4">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div className="md:col-span-4 space-y-1.5">
                        <label className="text-xs font-semibold text-neutral-500 uppercase">Imię i nazwisko / ID</label>
                        <Input
                    ...
                    <div className="md:col-span-3 space-y-1.5">
                        <label className="text-xs font-semibold text-neutral-500 uppercase">Płeć</label>
                        <select 
                            className="flex h-10 w-full items-center justify-between rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-950 disabled:cursor-not-allowed disabled:opacity-50"
                            value={patientSex}
                    ...
                    <div className="md:col-span-3 space-y-1.5">
                         <label className="text-xs font-semibold text-neutral-500 uppercase">Wiek</label>
                         <input 
                            type="number" 
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 print:flex print:flex-col print:gap-4">
            {traits.map(trait => {
                ...
                <Card 
                    key={trait.id} 
                    className={cn(
                        "border-0 shadow-sm ring-1 transition-all cursor-pointer hover:ring-2 print:ring-1 print:shadow-none print:break-inside-avoid print:bg-neutral-50 print:rounded-none print:break-after-page print:mb-8", 
                        isActive ? "ring-2 ring-neutral-900 scale-[1.02] print:scale-100 print:ring-neutral-300" : "ring-neutral-200 hover:ring-neutral-400"
                    )}
                    onMouseEnter={() => setActiveGraphTrait(trait.id)}
                    onTouchStart={() => setActiveGraphTrait(trait.id)}
                >
                    <CardHeader className="pb-2 print:p-0">
                        <CardTitle className="text-sm font-medium text-neutral-500 flex justify-between print:text-3xl print:font-bold print:text-neutral-900 print:mb-4">
                            {trait.name} 
                    ...
                    <CardContent className="print:p-0">
                        <div className="print:flex print:items-center print:gap-4 print:mb-6">
                            <div className="flex items-baseline gap-2">
                                <div className="text-3xl font-bold text-neutral-900 print:hidden">{rawScore}</div>
                                {sten > 0 && (
                                    <div className="text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded print:bg-transparent print:text-neutral-900 print:border print:border-neutral-200">
                                        Sten: {sten}
                                    </div>
                                )}
                            </div>
                            <div className="flex justify-between items-center mt-1 print:mt-0">
                                 <div className="text-xs text-neutral-400 print:hidden">Suma punktów</div>
                                 {sten > 0 && (
                                    <div className="text-[10px] uppercase font-bold text-neutral-400 tracking-wider print:text-sm print:font-semibold print:text-neutral-900 print:bg-transparent print:border print:normal-case print:border-neutral-200 print:px-2 print:py-0.5 print:rounded print:scale-100">
                                        Perc: {percentile}
                                    </div>
                                 )}
                            </div>
                        ...
                        <div className="mt-4 space-y-1 print:grid print:grid-cols-2 print:gap-x-8 print:gap-y-1">
                            {facets.filter(f => f.trait === trait.id).map(f => {
                                ...
                                return (
                                <div key={f.id} className="flex justify-between items-center text-xs py-0.5 border-b border-neutral-50 last:border-0 print:border-neutral-100">
                                    <span className="text-neutral-600 font-medium w-1/3 truncate print:w-auto" title={f.name}>{f.name}</span>
                                    <div className="flex gap-2 text-neutral-500 items-baseline">
                                        <span className="text-[10px] text-neutral-400 print:hidden">Raw:{fRaw}</span>
                                        {fStenData.sten > 0 && <span className="font-bold text-neutral-700">Sten:{fStenData.sten}</span>}
                                        {fStenData.percentile && <span className="text-[9px] text-emerald-600 font-medium print:text-neutral-500 print:inline-block">Perc: {fStenData.percentile}</span>}
                                    </div>
                                </div>
                            )})}
                        </div>
                        ...
                        <div className="hidden print:block mt-4 pt-4 border-t border-neutral-200">
        ...
        <Card className="border-0 shadow-sm ring-1 ring-neutral-200 print:hidden" onMouseEnter={() => setActiveGraphTrait(null)}>
             <CardHeader className="pb-3 border-b border-neutral-50 flex flex-row items-center justify-between">
            ...
            <CardContent className="pt-6 pb-6 flex justify-center bg-neutral-50/50 print:bg-white">
                 {(() => {
                     let items: any[] = [];
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden print:hidden">
             <div className="px-6 py-4 border-b border-neutral-100 font-semibold text-neutral-900">
                            ...
                            <div 
                                ...
                                className={cn(
                                    ...
                                    hasAnswer 
                                        ...
                                        : "bg-transparent border-neutral-100 text-neutral-300"
                                )}
                            >
                                <span className="font-bold mb-0.5 text-[10px] text-neutral-400">{num}</span>
                                <span className={cn("font-bold text-sm", hasAnswer ? "text-neutral-900" : "text-neutral-200")}>
                                    {hasAnswer ? answer : "·"}
                                </span>
                            </div>
                        )
